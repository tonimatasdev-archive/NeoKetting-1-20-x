...
public class Patcher {
    ...
    private void readInstallScript() {
        try(FileReader reader = new FileReader(KettingFileVersioned.NEOFORGE_INSTALL_JSON)){
    ...
    private void prepareTokens() {
        ...
        tokens.put("{PATCHED}", KettingFileVersioned.NEOFORGE_PATCHED_JAR.getAbsolutePath());
    ...
    private static void writeStoredHashes() throws IOException, NoSuchAlgorithmException {
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(KettingFiles.STORED_HASHES))){
            writer
                    .append("installJson=")
                    .append(HashUtils.getHash(KettingFileVersioned.NEOFORGE_INSTALL_JSON, "SHA3-512"))
                    .append("\nserverLzma=")
    ...
    public static boolean checkUpdateNeededInstallerJson(String mcVersion, String forgeVersion, String kettingVersion, String hash, boolean updating) throws Exception {
        ...
        final File forgeServerDir = new File(KettingFiles.KETTINGSERVER_NEOFORGE_DIR, mcForgeKettingVersion);
