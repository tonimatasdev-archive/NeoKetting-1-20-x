...
import org.kettingpowered.launcher.utils.JarUtils;
...
public class KettingLauncher {
    ...
    private static void extractBundledContent() throws Exception {
        ...
        final String fileName = "neoforge-"+version;
        final File folder = new File(KettingFiles.KETTINGSERVER_NEOFORGE_DIR, version);
        try{
            JarUtils.extractJarContent(KettingFiles.DATA_DIR+"ketting_libraries.txt", new File(folder, fileName+"-ketting-libraries.txt"));
        } catch (IOException e) {
            failed=true;
            exception.addSuppressed(e);
        }
        try{
            JarUtils.extractJarContent(KettingFiles.DATA_DIR+fileName+"-universal.jar", new File(KettingFiles.NEOFORGE_DIR, Bundled_ForgeVersion + "/neoforge-" + Bundled_ForgeVersion + "-universal.jar"));
        } catch (IOException e) {
            failed=true;
            exception.addSuppressed(e);
        }
        try {
            JarUtils.extractJarContent(KettingFiles.DATA_DIR + fileName + "-installscript.json", new File(folder, fileName + "-installscript.json"));
        } catch (IOException e) {
            failed = true;
            exception.addSuppressed(e);
        }
        try{
            JarUtils.extractJarContent(KettingFiles.DATA_DIR+"server.lzma", KettingFiles.SERVER_LZMA);
        } catch (IOException e) {
            failed=true;
            exception.addSuppressed(e);
        }
    ...
    private static void deleteOtherVersions(final String version){
        Arrays.stream(Objects.requireNonNullElse(KettingFiles.KETTINGSERVER_NEOFORGE_DIR.listFiles(File::isDirectory), new File[0]))
        ...
        Arrays.stream(Objects.requireNonNullElse(KettingFiles.NEOFORGE_DIR.listFiles(File::isDirectory), new File[0]))
                .filter(file -> !Objects.equals(file.getName(),Bundled_ForgeVersion))
                .forEach(FileUtils::deleteDir);
    }
    ...
    private void ensureOneServerAndUpdate(final String mc_version) {
        ...
        {
            File[] kettingServerVersions = Objects.requireNonNullElse(KettingFiles.KETTINGSERVER_NEOFORGE_DIR.listFiles(File::isDirectory), new File[0]);
    
            Logger.log(LogLevel.DEBUG, KettingFiles.KETTINGSERVER_NEOFORGE_DIR.getAbsolutePath());
            ...
            if(needsDownload) FileUtils.deleteDir(KettingFiles.KETTINGSERVER_NEOFORGE_DIR); //we have multiple ketting versions, but 0 that match the requested minecraft version.
        ...
        if (args.enableServerUpdator() || needsDownload) {
            ...
            final File forgeDir = new File(KettingFiles.KETTINGSERVER_NEOFORGE_DIR,mc_minecraft_forge);
    ...
    void prepareLaunch() throws Exception {
        ...
        downloadMCP.start();
        {
            ...
            try (BufferedReader stream = new BufferedReader(new FileReader(KettingFileVersioned.NEOFORGE_KETTING_LIBS))) {
        ...
        libs.addLoadedLib(new File(Main.LauncherDir, "libraries/org/kettingpowered/common/bundled/common-bundled.jar"));
